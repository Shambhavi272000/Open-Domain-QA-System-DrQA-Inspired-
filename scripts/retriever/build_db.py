...
"""This file is used for reading and storing documents in a sqlite database."""
...
def iter_files(path):
    """Going through all the files located below a root path."""
...
def get_contents(filename):
    """Deciphering the contents of a file. Each line of the file is a document encoded in JSON format."""
    global PREPROCESS_FN
    documents = []
    with open(filename) as f:
        for line in f:
            ...
            # Disregarding the document if it is empty or NONE
            if not doc:
                continue
            # Concatenating the document
            documents.append((utils.normalize(doc['id']), doc['text']))
    return documents


def store_contents(data_path, save_path, preprocess, num_workers=None):
    """Storing a corpus of documents in sqlite and preprocessing it
